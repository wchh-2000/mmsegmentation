...
model = dict(
    type='EncoderDecoder',
    pretrained='/data/checkpoints/convnext_tiny_22k_224.pth',
    backbone=dict(
        type='ConvNeXt',
        in_chans=3,
        depths=[3, 3, 9, 3],#[3, 3, 27, 3],
        dims=[96, 192, 384, 768],#[192, 384, 768, 1536],
        drop_path_rate=0.4,
        layer_scale_init_value=1.0,
        out_indices=[0, 1, 2, 3]),
    decode_head=dict(
        type='UPerHead',
        in_channels=[96, 192, 384, 768],#[192, 384, 768, 1536],
        in_index=[0, 1, 2, 3],
        pool_scales=(1, 2, 3, 6),
        channels=512,
        dropout_ratio=0.1,
...
evaluation = dict(interval=30, metric='mIoU', pre_eval=True)#type 与 runner一致
